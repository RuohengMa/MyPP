...
void Conv3dCooXPUKernel(const XPUContext& dev_ctx,
  ...
  // 4. scatter
  xpu::VectorParam<IntT> index{rulebook_ptr + n * 2, n, nullptr};
  ...
  *(out->mutable_values()) = phi::Cast<float, phi::XPUContext>(
      dev_ctx, tmp_out, phi::DataType::FLOAT64);
}
...
PD_REGISTER_KERNEL(
    conv3d_coo, XPU, ALL_LAYOUT, phi::sparse::Conv3dCooKernel, float, double) {
